...
# so devices should not inherit
# for it (if it walks like a duck,
# and quacks like a duck, it's a duck)
class Device(Protocol):
    async def connect(self) -> Awaitable[None]:
        ...  # Ellipsis - similar to
        # "pass", but sometimes has different meaning
...
class IOTService:
    ...
    async def run_program(self, program: list[Message]) -> None:
        await asyncio.to_thread(print, "=====RUNNING PROGRAM======")
        for msg in program:
            await self.send_msg(msg)
        await asyncio.to_thread(print, "=====END OF PROGRAM======")
